$(function () {
    ...
    function loadState(actionFunction) {
		...
		window.extraHashStore = savedHashState;
        $frames.empty();
        var frame;
	...
	function actOnLoadedState(actionFunction) {
        //savedHashState = window.location.hash.slice(1);
        $frames.empty();
        var frame;
		var newSavedHashedArray = [];
		//console.log(savedHashState);
        var patterns = window.extraHashStore.split('|');
        ...
        for (var i = 0; i < patterns.length; i++) {
			...
			console.log(actionFunction);
			console.log(pattern);
			newSavedHashedArray.push(pattern);
            frame = makeFrameElement(pattern);
 
            $frames.append(frame);
        }
		window.extraHashStore = newSavedHashedArray.join("|");
        frame.addClass('selected');
        $hexInput.val(frame.attr('data-hex'));
        printArduinoCode(newSavedHashedArray);
        hexInputToLeds();
    }
	...
	function oneLineUp(val) {
		var val = '00' + val.substr(0, 14);
		return val;
	}
	
	function oneLineDown(val) {
		var val =  val.substr(2, 14) + '00';
		return val;
	}
	
	function onePixelLeft(val) {
	    var out = [];
        for (var i = 0; i < 8; i++) {
	...
	*/
    ...
    $('#invert-button').click(function () {
        ...
        $hexInput.val(val);
		if(applyToAllStatus()) {
			actOnLoadedState(function(valIn){
				return invert(valIn);
			}) ;
		}	
        hexInputToLeds();
    ...
    $('#shift-right-button').click(function () {
        var val = getInputHexValue();
		val = onePixelRight(val);
        $hexInput.val(val);
		if(applyToAllStatus()) {
			actOnLoadedState(function(valIn){
				return onePixelRight(valIn);
			}) ;
		}	;
        hexInputToLeds();
    });
    ...
    $('#shift-left-button').click(function () {
        var val = getInputHexValue();
		val = onePixelLeft(val);
        $hexInput.val(val);
		if(applyToAllStatus()) {
			actOnLoadedState(function(valIn){
				return onePixelLeft(valIn);
			}) ;
		}	;
        hexInputToLeds();
    });
	...
	$('#flip-vertical-button').click(function () {
		...
		if(applyToAllStatus()) {
			actOnLoadedState(function(valIn){
				return verticalFlipCharacter(valIn);
			}) ;
		}	
        hexInputToLeds();
    });
	...
	$('#flip-horizontal-button').click(function () {
		...
		if(applyToAllStatus()) {
			actOnLoadedState(function(valIn){
				return horizontalFlipCharacter(valIn);
			}) ;
		}
        hexInputToLeds();
    });
    ...
    $('#rotate-counterclockwise-button').click(function () {
	 ...
	 $('#rotate-clockwise-button').click(function () {
		...
		if(applyToAllStatus()) {
			actOnLoadedState(function(valIn){
				return rotateCharacter(valIn, 1);
			}) ;
		}
    });
